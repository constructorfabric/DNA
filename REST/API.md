...
- **401 challenge**: every 401 MUST carry a `WWW-Authenticate: Bearer …` header

For the complete specification — token validation and JWKS rotation, scope naming, tenant
isolation, API keys, and the 403-vs-404 information-disclosure decision — see
[AUTH.md](AUTH.md).
...
```json
{
  ...
  "percent": 35,
  "result": { "resource_url": "/v1/reports/018f6c9e-5f60-7b7c-8d8e-9f0a1b2c3d4e" },
  "error": { "type": "...", "code": "...", "title": "...", "status": 500 },
  "expires_at": "2025-09-02T20:00:00.000Z",
  "created_at": "2025-09-01T20:00:00.000Z",
  "updated_at": "2025-09-01T20:03:12.500Z"
}
```

- **`error` is a Problem Details object** (see [§7](#7-error-model-problem-details)), not an
  untyped map. A failed job describes its failure exactly as a failed request would.
- **Result linkage**: when a job creates or modifies a resource, `result` MUST contain the
  URL of that resource. Otherwise the client has no way to get from a succeeded job to its
  output.
- **Polling**: clients poll `GET /jobs/{id}`. Responses to a non-terminal job MUST include
  `Retry-After` telling the client when to poll next; without it clients poll blindly and
  either hammer the service or lag behind it.
- **Cancellation**: `DELETE /jobs/{id}` requests cancellation → `202 Accepted`, and the job
  moves to `canceled` when the cancellation takes effect. Endpoints whose jobs cannot be
  cancelled MUST say so and return `405`.
- **Retention**: jobs are retained for a documented TTL, exposed as `expires_at`. After
  expiry, `GET /jobs/{id}` returns `410 Gone` — not `404`, which would be indistinguishable
  from a bad job id.
- **Streaming**: server-push delivery (SSE or WebSocket) is out of scope for this guideline.
  An API that offers it MUST specify the event shape and the reconnect/resume semantics in
...
- **Security**: `X-Signature` HMAC-SHA256 over a defined signature base string, with
  `X-Timestamp` (±5 min skew). The exact base string — which bytes, in which order, with
  which separator — is specified in [WEBHOOKS.md](WEBHOOKS.md); receivers cannot verify
  anything without it.
- **Retries**: Exponential backoff for ≥24h; dead-letter queue with a retrieval endpoint
- **Idempotency**: receivers dedupe on `id`
- **Ordering**: delivery is **not** ordered. Receivers MUST tolerate out-of-order events.

For the complete specification — signature base string and worked example, key rotation,
replay protection, subscription management, event-type naming and versioning, and
...
- Client SDK: generate TypeScript types (`openapi-typescript`) and React hooks (TanStack
  Query) with fetch/axios adapter
- Keep schemas DRY via shared components. This repository publishes the shared ones so they
  do not have to be retyped: [`schemas/components.openapi.yaml`](../schemas/components.openapi.yaml)
  (`PageInfo`, `Problem`, `ValidationError`, and the five query parameters) and
  [`schemas/problem.schema.json`](../schemas/problem.schema.json). `$ref` them.
- Validate the generated document against [`schemas/spectral.yaml`](../schemas/spectral.yaml)
  in CI; the checkable norms of this guideline are encoded there.
...
- The 1 MB JSON payload cap of [§23](#23-performance--dos) does **not** apply to binary
  uploads; upload endpoints document their own limit.
- The client-declared `Content-Type` MUST NOT be trusted. Sniff the content and enforce an
  allow-list of accepted types.
- Download URLs are short-lived and pre-signed; no permanently public object URLs.

For the complete specification — resumable uploads, scanning, size limits, expiry, and the
interaction with `Idempotency-Key` and async jobs — see [UPLOADS.md](UPLOADS.md).
