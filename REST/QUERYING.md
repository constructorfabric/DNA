...
Reusable definitions for these parameters, for `page_info` and for `Problem` are published in [`schemas/components.openapi.yaml`](../schemas/components.openapi.yaml); `$ref` them rather than retyping them.
